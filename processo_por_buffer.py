...
def collect_urls_per_collection(stac_response, tipo=None):
    urls_per_collection = {}
    seen_urls = set()
    for item in stac_response.get("features", []):
        collection = item.get("collection", "unknown")
...
def main(bbox, stac_url, output_dir, delay, collections=None, tipo=None, nome_id=None):
    ...
    for collection, url_id_ext_pairs in all_urls_per_collection.items():
        ...
        if tipo == "MDS" and "MDS-50cm" not in collection:
            continue
        if tipo == "LAZ" and "LAZ" not in collection:
            continue
    ...
    # --- Merge dos rasters MDS-50cm se tipo for MDS ou ambos ---
    if tipo in (None, "MDS"):
        import rasterio
        from rasterio.merge import merge
        import glob
        # Procurar pasta MDS-50cm
        mds_dir = os.path.join(output_dir, 'MDS-50cm')
        if os.path.exists(mds_dir):
            tif_files = glob.glob(os.path.join(mds_dir, '*.tif'))
            if len(tif_files) > 1:
                ...
                # Nome do raster unido: nome_id.tif
                nome_raster = f"{nome_id}.tif" if nome_id else "MDS-50cm_merged.tif"
...
def main_popup_and_shapefile():
    ...
    if dlg.choice == 'sado':
        ...
        gdf_buffer = gpd.GeoDataFrame({'SADO': [sado_input]}, geometry=buffer, crs=utm_crs)
        buffer_shp = os.path.join(shp_folder, f'buffer_{sado_input}_{buffer_size//1000}km.shp')
        gdf_buffer.to_crs("EPSG:4326").to_file(buffer_shp)
    ...
    # Não mostrar coleções, nem pedir input
    print("\nInício do processo de download apenas de MDS-50cm e LAZ...\n")
    tipo = None
    for arg in sys.argv:
        if arg.startswith('--tipo='):
            tipo = arg.split('=')[1].upper()
    main(bbox, STAC_SEARCH_URL, output_dir, delay, collections=None, tipo=tipo, nome_id=nome_id)
